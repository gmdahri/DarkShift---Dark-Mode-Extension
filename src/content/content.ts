...
import { toggleReadingMode, isReadingModeActive } from './readingMode';
...
async function initFAB() {
  try {
    const result = await chrome.storage.sync.get([STORAGE_KEYS.SHOW_FLOATING_BUTTON]);
    fabEnabled = result[STORAGE_KEYS.SHOW_FLOATING_BUTTON] ?? true;
...
function setupSystemThemeListener() {
  ...
  mediaQuery.addEventListener('change', async (e) => {
    try {
      const result = await chrome.storage.sync.get([STORAGE_KEYS.FOLLOW_SYSTEM_THEME]);
...
async function checkSchedule(): Promise<boolean> {
  try {
    const result = await chrome.storage.sync.get([STORAGE_KEYS.SCHEDULE_SETTINGS]);
...
chrome.runtime.onMessage.addListener(async (message, _sender, sendResponse) => {
  try {
    if (message.type === MESSAGE_TYPES.TOGGLE_DARK_MODE) {
      ...
      const settings = message.payload as Partial<DarkModeSettings>;
      ...
      hideFAB();
      sendResponse({ success: true });
    } else if (message.type === MESSAGE_TYPES.TOGGLE_READING_MODE) {
      const result = toggleReadingMode();
      sendResponse({ success: true, active: result });
    } else if (message.type === MESSAGE_TYPES.GET_READING_MODE_STATE) {
      sendResponse({ active: isReadingModeActive() });
    }
  } catch (error) {
